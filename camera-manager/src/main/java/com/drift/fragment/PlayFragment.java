...
import com.drift.camcontroldemo.LinkCamDetailActivity;
...
//        , PhotoViewAttacher.OnMatrixChangedListener
{
    ...
    public static final String KEY = "EasyPlayer is free!";
    ...
    private int mRatioType = ASPECT_RATIO_CENTER_CROPS;//ASPECT_RATIO_INSIDE;
...
//    private AngleView mAngleView;
    ...
    private SurfaceTexture mSurfaceTextureSave;
    ...
    private String recordFilePathToScan;
...
//                    }
                    ...
                    if (activity instanceof LinkCamDetailActivity) {
                        int state = resultData.getInt("state");
                        String msg = resultData.getString("event-msg");
                        ((LinkCamDetailActivity) activity).onEvent(PlayFragment.this, state, errorCode, msg);
                    }
                } else if (resultCode == EasyPlayerClient.RESULT_RECORD_BEGIN) {
                    if (activity instanceof LinkCamDetailActivity)
                        ((LinkCamDetailActivity)activity).onRecordState(1);
...
//        Toast.makeText(PlayActivity.this, "视频正在播放了", Toast.LENGTH_SHORT).show();
    ...
    public boolean onRecordOrStop() {
        if (!mStreamRender.isRecording()) {
            recordFilePath = FileUtil.getMovieName(mUrl).getPath();
            mStreamRender.startRecord(recordFilePath);
            Log.d(TAG, "drift test: come to start record.");
            return true;
        } else {
            mStreamRender.stopRecord();
            Log.d(TAG, "drift test: come to stop record.");
            recordFilePathToScan = recordFilePath;
            Uri uri = Uri.parse(recordFilePathToScan);
            MediaScannerConnection.scanFile(getContext(), new String[]{uri.toString()}, null,
    ...
    {
        if(mSurfaceTextureSave!=null)
        {
            View view = getView();
            view.findViewById(android.R.id.progress).setVisibility(View.VISIBLE);
            startRending(mSurfaceTextureSave);
        }
    }
    ...
    protected void startRending(SurfaceTexture surface) {
        ...
        boolean autoRecord = false;//SPUtil.getAutoRecord(getContext());
